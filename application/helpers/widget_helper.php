...
if (!function_exists('announcements_alert_widget')) {
    ...
    function announcements_alert_widget($return_as_data = false) {
        $ci = get_instance();
$announcements = $ci->Announcements_model->get_unread_announcements(
    $ci->login_user->id,$ci->login_user->team_ids, $ci->login_user->user_type)->result();
        $view_data["announcements"] = $announcements;
        return $ci->load->view("announcements/alert", $view_data, $return_as_data);
    }
...
if (!function_exists('announcements_marquee_widget')) {

    function announcements_marquee_widget($return_as_data = false) {
        $ci = get_instance();
$announcements_marquee = $ci->Announcements_model->get_marquee_announcements(
    $ci->login_user->id,$ci->login_user->team_ids, $ci->login_user->user_type)->result();
        $view_data["marquee_announcements"] = $announcements_marquee;
        return $ci->load->view("announcements/marquee", $view_data, $return_as_data);
    }
...
if (!function_exists('all_tasks_kanban_widget')) {
    ...
    function all_tasks_kanban_widget($return_as_data = false) {
        ...
        // active team memebers and inactive team members dropdown
        $active_team_members_dropdown = array(array("id" => "", "text" => "- " . lang("active_members") . " -"));
        $active_users_list = $ci->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "user_type" => "staff","status"=>"active"));
        ...
        $inactive_users_list = $ci->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "user_type" => "staff","status"=>"inactive"));
