...
if (!function_exists('get_notification_config')) {
    ...
    function get_notification_config($event = "", $key = "", $info_options = array()) {
        ...
        $employee_payslip_link = function($options) {
            $url = "";
            if (isset($options->payslip_id)) {
                $url = get_uri("payslip/preview/" . $options->payslip_id);
            }

            return array("url" => $url);
        }; 

        $voucher_link = function($options) {
            $url = "";
            if (isset($options->voucher_id)) {
                $url = get_uri("voucher/preview/" . $options->voucher_id);
            }

            return array("url" => $url);
        }; 

        $dc_link = function($options) {
            $url = "";
            if (isset($options->dc_id)) {
                $url = get_uri("delivery/preview/" . $options->dc_id);
            }

            return array("url" => $url);
        };

        $purchase_order_link = function($options) {
            $url = "";
            if (isset($options->purchase_order_id)) {
                $url = get_uri("purchase_orders/preview/" . $options->purchase_order_id);
            }

            return array("url" => $url);
        }; 

        $group_link = function($options) {
            $url = "";
            if (isset($options->group_id)) {
                if ($options->event == "group_comment_added" || $options->event == "group_comment_replied") {
                    $message_id = isset($options->group_id) && $options->group_id ? $options->group_id : $options->group_id;
                    $url = get_uri("messages/groups_items/" . $message_id);
                }
            }
        ...
        $events = array(
            ...
            "leave_application_submitted" => array(
                "notify_to" => array("team_members", "team","leave_manager"),
                "info" => $leave_link
            ),
            "leave_approved" => array(
                "notify_to" => array("leave_applicant", "team_members", "team","leave_manager"),
                "info" => $leave_link
            ),
            "leave_approved_by_manager" => array(
                "notify_to" => array("leave_applicant", "team_members", "team", "admin", "hr_department"),
                "info" => $leave_link
            ),
            "leave_alternate" => array(
                "notify_to" => array("leave_alternate"),
                "info" => $leave_link
            ),   
            ...
            "generate_employee_payslip" => array(
                "notify_to" => array("employee_payslip_applicant"),
                "info" => $employee_payslip_link
            ),
            "voucher_application_submitted" => array(
                "notify_to" => array("line_manager"),
                "info" => $voucher_link
            ),
            "voucher_application_resubmitted" => array(
                "notify_to" => array("line_manager"),
                "info" => $voucher_link
            ),
            "voucher_application_payment_done" => array(
                "notify_to" => array("voucher_creater","line_manager","account_department"),
                "info" => $voucher_link
            ),
            "voucher_application_approved_by_manager" => array(
                "notify_to" => array("voucher_application_approved_by_manager","account_department"),
                "info" => $voucher_link
            ),
            "voucher_application_approved_by_accounts" => array(
                "notify_to" => array("voucher_application_approved_by_accounts"),
                "info" => $voucher_link
            ),
            "voucher_application_rejected_by_manager" => array(
                "notify_to" => array("voucher_application_rejected_by_manager"),
                "info" => $voucher_link
            ),
            "voucher_application_rejected_by_accounts" => array(
                "notify_to" => array("voucher_application_rejected_by_accounts"),
                "info" => $voucher_link
            ),  
            "voucher_application_payment_in_progress" => array(
                "notify_to" => array("voucher_creater","line_manager","account_department"),
                "info" => $voucher_link
            ), 
            "voucher_application_payment_hold" => array(
                "notify_to" => array("voucher_creater","line_manager","account_department"),
                "info" => $voucher_link
            ),
            "voucher_application_payment_received" => array(
                "notify_to" => array("voucher_creater","line_manager","account_department"),
                "info" => $voucher_link
            ), 
            "voucher_application_closed" => array(
                "notify_to" => array("voucher_creater","line_manager","account_department"),
                "info" => $voucher_link
            ),
            "delivery_chellan_submitted" => array(
                "notify_to" => array("team_members", "team"),
                "info" => $dc_link
            ),
            "purchase_order_due_reminder_before_due_date" => array(
                "notify_to" => array("team_members", "team"),
                "info" => $purchase_order_link
            ),
             "purchase_order_overdue_reminder" => array(
                "notify_to" => array("team_members", "team"),
                "info" => $purchase_order_link
            ),
            "group_comment_added" => array(
                "notify_to" => array("group_members", "team_members", "team"),
                "info" => $group_link
            ),
             "group_comment_replied" => array(
                "notify_to" => array("group_members", "group_comment_creator", "team_members", "team"),
                "info" => $group_link
            ),
        );
...
if (!function_exists('send_notification_emails')) {
    ...
    function send_notification_emails($notification_id, $email_notify_to, $extra_data = array()) {
        ...
        if ($notification->category == "ticket") {
            ...
            $email_options["attachments"] = array(array("file_path" => $attachement_url));
        }else if ($notification->event == "purchase_order_due_reminder_before_due_date"||$notification->event == "purchase_order_overdue_reminder") {
            if ($notification->event == "purchase_order_due_reminder_before_due_date") {
               $email_template = $ci->Email_templates_model->get_final_template("purchase_order_due_reminder_before_due_date");
            }else if($notification->event == "purchase_order_overdue_reminder"){
                $email_template = $ci->Email_templates_model->get_final_template("purchase_order_overdue_reminder");
            }
            

            $purchase_order_data = get_purchase_order_making_data($notification->purchase_order_id);
            $purchase_order_info = get_array_value($purchase_order_data, "purchase_order_info");
            //$invoice_total_summary = get_array_value($invoice_data, "invoice_total_summary");

            //$primary_contact = $ci->Clients_model->get_primary_contact($invoice_info->client_id, true);

            $parser_data["PURCHASE_ORDER_ID"] = $notification->purchase_order_id;
            /*$parser_data["CONTACT_FIRST_NAME"] = $primary_contact->first_name;
            $parser_data["CONTACT_LAST_NAME"] = $primary_contact->last_name;*/
            /*$parser_data["BALANCE_DUE"] = to_currency($invoice_total_summary->balance_due, $invoice_total_summary->currency_symbol);*/
            $parser_data["DUE_DATE"] = format_to_date($purchase_order_info->valid_until, false);
            //$parser_data["PROJECT_TITLE"] = $invoice_info->project_title;
            $parser_data["PURCHASE_ORDER_URL"] = $url;


            $attachement_url = prepare_purchase_order_pdf( $purchase_order_data, "send_email");
            $email_options["attachments"] = array(array("file_path" => $attachement_url));
        }else if ($notification->event == "generate_employee_payslip"){
            $email_template = $ci->Email_templates_model->get_final_template("general_notification");

            $payslip_data = get_payslip_making_data($notification->payslip_id);
            $payslip_info = get_array_value($payslip_data, "payslip_info");
            $parser_data["EVENT_TITLE"] = "<b>" . $notification->user_name . "</b> " . sprintf(lang("notification_" . $notification->event), $notification->to_user_name);
            $parser_data["NOTIFICATION_URL"] = $url;

            $attachement_url = prepare_payslip_pdf($payslip_data, "send_email");
            $email_options["attachments"] = array(array("file_path" => $attachement_url));
            $view_data["notification"] = $notification;
            $parser_data["EVENT_DETAILS"] = $ci->load->view("notifications/notification_description", $view_data, true);
        }else if ($notification->event == "voucher_application_submitted"){
            $email_template = $ci->Email_templates_model->get_final_template("general_notification");

            $voucher_data = get_voucher_making_data($notification->voucher_id);
            $voucher_info = get_array_value($voucher_data, "estimate_info");
            $parser_data["EVENT_TITLE"] = "<b>" . $notification->user_name . "</b> " . sprintf(lang("notification_" . $notification->event), $notification->to_user_name);
            $parser_data["NOTIFICATION_URL"] = $url;
