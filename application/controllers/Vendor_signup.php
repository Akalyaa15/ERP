...
class Vendor_signup extends CI_Controller {
    ...
    function index() {
        ...
        $view_data["signup_message"] = lang("create_an_account_as_a_new_vendor");
    ...
    function create_account() {
        ...
        if ($signup_key) {
            ...
            if ($valid_key) {
                ...
                $email = get_array_value($valid_key, "email");
                $type = get_array_value($valid_key, "type");
                $vendor_id = get_array_value($valid_key, "vendor_id");
                ...
                if ($type === "staff") {
                    ...
                    if (isset($vendor->id) && $vendor->deleted == 0) {
            ...
            $vendor_data = array("company_name" => $company_name);

            $vendor_data = clean_data($vendor_data);

            //check duplicate company name, if found then show an error message
            if (get_setting("disallow_duplicate_client_company_name") == "1" && $this->Vendors_model->is_duplicate_company_name($company_name)) {
            ...
            //create a client
            $vendor_id = $this->Vendors_model->save($vendor_data);
            if ($vendor_id) {
