...
class Vendors_register extends MY_Controller {
    ...
    function index() {
        $view_data["custom_field_headers"] = $this->Custom_fields_model->get_custom_field_headers_for_table("vendors", $this->login_user->is_admin, $this->login_user->user_type);
        $view_data['groups_dropdown'] = json_encode($this->_get_groups_dropdown_select2_data(true));

        // Initialize $show_invoice_info and pass it to the view
        $view_data['show_invoice_info'] = true; // Set it to true or false based on your logic

        $this->template->rander("vendors_register/index", $view_data);
    }
    ...
    function modal_form() {
        ...
        $this->load->view('vendors_register/modal_form', $view_data);
    }
   ...
   private function _get_state_dropdown_select2_data($show_header = false) {
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $group_list = "";
        if (isset($data->groups) && $data->groups) { // Ensure $data->groups is set
        ...
        $due = 0;
        if (isset($data->purchase_order_value) && $data->purchase_order_value) {
            $due = ignor_minor_value($data->purchase_order_value - $data->payment_received);
        }
    
        $row_data = array($data->id,
            anchor(get_uri("vendors_register/view/" . $data->id), $data->company_name),
            isset($data->primary_contact) ? $primary_contact : "",
            $group_list,
        );
        ...
        $row_data[] = modal_anchor(get_uri("vendors_register/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_vendor'), "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_vendor'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("vendors_register/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($vendor_id = 0, $tab = "") {
        ...
        if ($vendor_id) {
            ...
            if ($vendor_info) {
                ...
                $this->template->rander("vendors_register/view", $view_data);
            } else {
                show_404();
            }
        } else {
