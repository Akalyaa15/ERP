...
class Invoice_payments extends MY_Controller {
    ...
    function save_payment() {
        ...
        $files_data = move_files_from_temp_dir_to_permanent_dir($target_path, "invoice_payment");
        ...
        if ($id) {
            $payment_info = $this->Invoice_payments_model->get_one($id);
        ...
        if($invoice_payment_data["files"]=='a:0:{}'){
    ...
    private function _make_payment_row($data) {
       // $invoice_url = "";
        ...
        $invoice_table_list =$this->Invoices_model->get_one($data->invoice_id); 
        $invoice_no_value =  $invoice_table_list->invoice_no ?  $invoice_table_list->invoice_no: get_invoice_id($data->invoice_id);
        $invoice_no_url = "";
        if ($this->login_user->user_type == "staff") {
             $invoice_no_url = anchor(get_uri("invoices/view/" . $data->invoice_id), $invoice_no_value);
        } else {
             $invoice_no_url = anchor(get_uri("invoices/preview/" . $data->invoice_id), $invoice_no_value);
        }

        //get project title
        $project_table_list =$this->Projects_model->get_one($invoice_table_list->project_id); 
        $client_table_list =$this->Clients_model->get_one($invoice_table_list->client_id); 
        ...
        if ($data->files) {
            ...
            if (count($files)) {
                foreach ($files as $file) {
                    ...
                    $files_link .= js_anchor(" ", array('title' => "", "data-toggle" => "app-modal", "data-sidebar" => "0", "class" => "pull-left font-22 mr10 $link", "title" => remove_file_prefix($file_name), "data-url" => get_uri("invoice_payments/file_preview/" . $file_name)));
                }
            }
        }
        return array(
            //$invoice_url,
             $invoice_no_url,
              $client_table_list->company_name? anchor(get_uri("clients/view/" . $invoice_table_list->client_id), $client_table_list->company_name):"-",
             $project_table_list->title ? anchor(get_uri("projects/view/" . $invoice_table_list->project_id), $project_table_list->title) : "-",
            $data->payment_date,
