...
class Tickets extends MY_Controller {
    ...
    function index() {
        ...
        if ($this->login_user->user_type === "staff") {
            ...
            $assigned_to_list = $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "user_type!=" => "client", "client_id" => 0));
            ...
            $assigned_to_list = $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "user_type" => "resource"));
            foreach ($assigned_to_list as $key => $value) {
                $assigned_to_dropdown[] = array("id" => $key, "text" => $value);
            }
...
if ($this->login_user->user_type === "resource" && $this->access_type !== "all") {
            $where = array("where_in" => array("id" => $this->allowed_ticket_types));
        }
        ...
        $view_data['ticket_types_dropdown'] = $this->Ticket_types_model->get_dropdown_list(array("title"), "id",array("status"=>"active"), $where);
        ...
        $assigned_to_dropdown = array("" => "-") + $this->Users_model->get_dropdown_list(array("first_name", "last_name"), "id", array("deleted" => 0, "user_type!=" => "client", "client_id" => 0));
    ...
    private function _make_row($data, $custom_fields) {
        ...
        if ($data->assigned_to && $this->login_user->user_type == "staff") {
            ...
            if ($data->assigned_to_user_type=="staff") {
            $assigned_to = get_team_member_profile_link($data->assigned_to, $assigned_to_user);
            }
if ($data->assigned_to_user_type=="resource") {
            $assigned_to = get_rm_member_profile_link($data->assigned_to, $assigned_to_user);
            }
        }
if ($data->assigned_to && $this->login_user->user_type == "resource") {
            $image_url = get_avatar($data->assigned_to_avatar);
            $assigned_to_user = "<span class='avatar avatar-xs mr10'><img src='$image_url' alt='...'></span> $data->assigned_to_user";
             if ($data->assigned_to_user_type=="staff") {
            $assigned_to = get_team_member_profile_link($data->assigned_to, $assigned_to_user);
...
if ($data->assigned_to_user_type=="resource") {
        ...
        //created by user name 
         $created_options = array("id"=>$data->created_by);
        $created_user_data = $this->Users_model->get_details($created_options)->row();
        $create_image_url = get_avatar($created_user_data->image);
        $created_user = "<span class='avatar avatar-xs mr10'><img src='$create_image_url' alt='...'></span> $created_user_data->first_name $created_user_data->last_name";
         $created_by_user_name= "-";
        if($created_user_data->user_type=="resource"){
          $created_by_user_name= get_rm_member_profile_link($data->created_by, $created_user );   
        }else if($created_user_data->user_type=="client") {
          $created_by_user_name= get_client_contact_profile_link($data->created_by, $created_user);
        }else if($created_user_data->user_type=="staff"){
             $created_by_user_name= get_team_member_profile_link($data->created_by, $created_user ); 
       }
