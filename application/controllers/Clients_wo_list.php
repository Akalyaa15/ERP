...
class Clients_wo_list extends MY_Controller {
    ...
    function index() {
        ...
        // Check user type and access level
        if ($this->login_user->is_admin == "1") {
            // Admin specific data
            $this->template->rander("clients_wo_list/index", $view_data);
    ...
    function yearly() {
        $this->load->view("clients_wo_list/yearly_clients_wo_list");
    }
...
//$this->access_only_allowed_members();
       ...
       $client_id = $this->input->post('client_id');
        $view_data["vendor_id"] = $client_id;
        $model_info = $this->Clients_wo_list_model->get_one($this->input->post('id'));
        $view_data['model_info'] =  $model_info;
        $view_data['vendors_dropdown'] = array("" => "-") + $this->Clients_model->get_dropdown_list(array("company_name"));
         ...
         //$view_data['statuses'] = $this->Vendors_invoice_status_model->get_details()->result();
          //voucher id dropdown 
        $po_info = $this->Estimates_model->get_one($model_info->purchase_order_id);
        $purchase_id_dropdown = array(array("id" => "", "text" => "-"));
        $purchase_id_dropdown[] = array("id" => $model_info->purchase_order_id, "text" => $po_info->estimate_no?$po_info->estimate_no:get_estimate_id($model_info->purchase_order_id));
        $view_data['purchase_id_dropdown'] = $purchase_id_dropdown;
        $this->load->view('clients_wo_list/modal_form', $view_data);
    }
 ...
 //gst state code
    ...
    function save() {
        ...
        $files_data = move_files_from_temp_dir_to_permanent_dir($target_path, "client_wo");
...
"purchase_order_id"=>$this->input->post('purchase_order_id'),
        ...
        if ($id) {
            $note_info = $this->Clients_wo_list_model->get_one($id);
    ...
    // check the vendor invoice no     
        ...
        if ($this->Clients_wo_list_model->is_vendors_invoice_exists($data["invoice_no"])) {
                echo json_encode(array("success" => false, 'message' => lang('client_wo_already')));
                exit();
            }
    ...
    // check the vendor invoice no     
       ...
       if ($this->Clients_wo_list_model->is_vendors_invoice_exists($data["invoice_no"],$id)) {
                echo json_encode(array("success" => false, 'message' => lang('client_wo_already')));
                exit();
            }
    ...
    exit();
        ...
        $save_id = $this->Clients_wo_list_model->save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Clients_wo_list_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        } else {
            if ($this->Clients_wo_list_model->delete($id)) {
    ...
    function list_data() {
       ...
       /* $options = array(
         ...
         $list_data = $this->Clients_wo_list_model->get_details( $options)->result();
         $result = array();
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Clients_wo_list_model->get_details($options)->row();
    ...
    private function _make_row($data) {
        ...
        if ($data->files) {
            ...
            $files_links = "";
            $payment_files = $this->Clients_wo_payments_list_model->get_details(array("task_id" => $data->id))->result();
...
if($data->purchase_order_id){
    $purchase_info = $this->Estimates_model->get_one($data->purchase_order_id);
     $purchase_order_url = anchor(get_uri("estimates/view/" . $data->purchase_order_id), $purchase_info->estimate_no?$purchase_info->estimate_no:get_estimate_id($data->purchase_order_id));
 }else{
    $purchase_order_url = "-";
 }
       $due = 0;
        ...
        return array(//$data->id,
            ...
            $data->invoice_date,
            //$data->vendor_name,
            anchor(get_uri("clients/view/" . $data->vendor_id), $data->vendor_name),
            $purchase_order_url,
           ...
           // $status,
            ...
            modal_anchor(get_uri("clients_wo_list/task_view"), "<i class='fa fa-pencil'></i>", array("class" => "add_payment", "title" => lang('add_payment'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_tax'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("clients_wo_list/delete"), "data-action" => "delete-confirmation"))
        );
    }
...
private function _get_vendor_invoice_status_label($data, $return_html = true) {
    ...
    function task_view() {
        ...
        $model_info = $this->Clients_wo_list_model->get_details(array("id" => $task_id))->row();
        if (!$model_info->id) {
            show_404();
        }
        ...
        $checklist_items = $this->Clients_wo_payments_list_model->get_details(array("task_id" => $task_id))->result();
     ...
     $view_data['payment_methods_dropdown'] = $this->Payment_methods_model->get_dropdown_list(array("title"), "id", array("online_payable" => 0, "deleted" => 0));
        ...
        $this->load->view('clients_wo_list/view', $view_data);
    }


    function get_vendors_invoice_no_suggestion() {
        $item = $this->Clients_wo_list_model->get_invoice_no_suggestion($this->input->post("item_name"));
        if ($item) {
 ...
 /* checklist */
    ...
    function save_checklist_item() {
        ...
        $data = array(
            ...
            $note_info = $this->Clients_wo_payments_list_model->get_one($id);
          $timeline_file_path = get_setting("timeline_file_path");
    ...
    exit();
        ...
        $save_id = $this->Clients_wo_payments_list_model->save($data);

        if ($save_id) {
            $item_info = $this->Clients_wo_payments_list_model->get_one($save_id);
    ...
    private function _make_checklist_item_row($data = array(), $return_type = "row") {
        ...
        $is_checked_value = 1;
         ...
         $client_wo_info = $this->Clients_wo_list_model->get_one($data->task_id);
         $client_info = $this->Clients_model->get_one($client_wo_info->vendor_id);
...
if ($data->is_checked == 1) {
        $title = "<span class='font-13 '>" .to_currency( $data->title,$client_info->currency_symbol)." , "."Payment Date:".$data->payment_date." ". $files_link." , ".$payment_title_info->title." No:".$data->reference_number.","."Payment Mode:".$payment_title_info->title." , "."Decription:".$data->description." ". $title_class. "</span>";
    }
...
$delete = js_anchor("<i class='fa fa-times pull-right p3'></i>", array('title' => lang('delete_income'), "class" => "delete-checklist-item", "data-fade-out-on-success" => "#checklist-item-row-$data->id", "data-action-url" => get_uri("clients_wo_list/delete_checklist_item/$data->id"), "data-action" => "delete-confirmation"));
    ...
    $task_id = $this->Clients_wo_payments_list_model->get_one($id)->task_id;
...
$save_id = $this->Clients_wo_payments_list_model->save($data, $id);
    ...
    function save_checklist_items_sort() {
        ...
        if ($sort_values) {
            ...
            $this->Clients_wo_payments_list_model->save($data, $id);
            }
        }
    }
    ...
    function delete_checklist_item($id) {
        ...
        if ($this->Clients_wo_payments_list_model->delete($id)) {
    ...
    function get_vendors_invoice_paid_suggestion() {
        $item = $this->Clients_wo_list_model->get_vendors_invoice_paid_amount_suggestion($this->input->post("item_name"));
        if ($item) {
            echo json_encode(array("success" => true, "item_info" => $item));
        } else {
            echo json_encode(array("success" => false));
...
$loan_voucher_no=str_ireplace("]",")",$vv);
    ...
    $loan_voucher_no="('empty')";
        ...
        $itemss = $this->Clients_wo_list_model->get_purchase_orderid($this->input->post("vendor_member"),$loan_voucher_no);
         $suggestions = array();
      foreach ($itemss as $items) {
           $suggestions[] = array("id" => $items->id, "text" => $items->estimate_no?$items->estimate_no:get_estimate_id($items->id)/*.'['.$items->title.']'*/);
       }
