...
//$this->access_only_allowed_members();
        ...
        $view_data['model_info'] = $this->Vendors_invoice_list_model->get_one($this->input->post('id'));
...
if ($data->is_checked == 1) {
        $title = "<span class='font-13 '>" .to_currency( $data->title,$data->currency_symbol)." , "."Payment Date:".$data->payment_date." ". $title_class. "</span>";
    }

    if ($data->is_checked == 0) {
        $title = "<span class='font-13 '>" .to_currency( $data->title,$data->currency_symbol)." , "."Payment Date:".$data->payment_date. "</span>";
