...
class Clients_po_list extends MY_Controller {
  ...
  //get clients dropdown
    ...
    function yearly() {
        $this->load->view("clients_po_list/yearly_clients_po_list");
    }
...
//$this->access_only_allowed_members();
        ...
        $model_info = $this->Clients_po_list_model->get_one($this->input->post('id'));
        ...
        $this->load->view('clients_po_list/modal_form', $view_data);
    }
 ...
 //gst state code
    ...
    function save() {
        ...
        $files_data = move_files_from_temp_dir_to_permanent_dir($target_path, "client_po");
...
"purchase_order_id"=>$this->input->post('purchase_order_id'),
        ...
        if ($id) {
            $note_info = $this->Clients_po_list_model->get_one($id);
    ...
    // check the vendor invoice no     
        ...
        if ($this->Clients_po_list_model->is_vendors_invoice_exists($data["invoice_no"])) {
                echo json_encode(array("success" => false, 'message' => lang('client_po_already')));
                exit();
            }
    ...
    // check the vendor invoice no     
       ...
       if ($this->Clients_po_list_model->is_vendors_invoice_exists($data["invoice_no"],$id)) {
                echo json_encode(array("success" => false, 'message' => lang('client_po_already')));
                exit();
            }
    ...
    exit();
        ...
        $save_id = $this->Clients_po_list_model->save($data, $id);
        if ($save_id) {
    ...
    function delete() {
        ...
        if ($this->input->post('undo')) {
            if ($this->Clients_po_list_model->delete($id, true)) {
                echo json_encode(array("success" => true, "data" => $this->_row_data($id), "message" => lang('record_undone')));
            } else {
                echo json_encode(array("success" => false, lang('error_occurred')));
            }
        } else {
            if ($this->Clients_po_list_model->delete($id)) {
    ...
    function list_data() {
       ...
       /* $options = array(
         ...
         $list_data = $this->Clients_po_list_model->get_details( $options)->result();
         $result = array();
    ...
    private function _row_data($id) {
        $options = array("id" => $id);
        $data = $this->Clients_po_list_model->get_details($options)->row();
    ...
    private function _make_row($data) {
        ...
        $files_links = "";
            ...
            $payment_files = $this->Clients_po_payments_list_model->get_details(array("task_id" => $data->id))->result();
       ...
       $due = 0;
        ...
        return array(//$data->id,
           ...
           // $status,
            ...
            modal_anchor(get_uri("clients_po_list/task_view"), "<i class='fa fa-pencil'></i>", array("class" => "add_payment", "title" => lang('add_payment'), "data-post-id" => $data->id))
            . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_tax'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("clients_po_list/delete"), "data-action" => "delete-confirmation"))
        );
    }
...
private function _get_vendor_invoice_status_label($data, $return_html = true) {
    ...
    function task_view() {
        ...
        $model_info = $this->Clients_po_list_model->get_details(array("id" => $task_id))->row();
        if (!$model_info->id) {
            show_404();
        }
        ...
        $checklist_items = $this->Clients_po_payments_list_model->get_details(array("task_id" => $task_id))->result();
     ...
     $view_data['payment_methods_dropdown'] = $this->Payment_methods_model->get_dropdown_list(array("title"), "id", array("online_payable" => 0, "deleted" => 0));
        ...
        $this->load->view('clients_po_list/view', $view_data);
    }


    function get_vendors_invoice_no_suggestion() {
        $item = $this->Clients_po_list_model->get_invoice_no_suggestion($this->input->post("item_name"));
        if ($item) {
 ...
 /* checklist */
    ...
    function save_checklist_item() {
        ...
        $data = array(
            ...
            $note_info = $this->Clients_po_payments_list_model->get_one($id);
          $timeline_file_path = get_setting("timeline_file_path");
    ...
    exit();
        ...
        $save_id = $this->Clients_po_payments_list_model->save($data);

        if ($save_id) {
            $item_info = $this->Clients_po_payments_list_model->get_one($save_id);
    ...
    private function _make_checklist_item_row($data = array(), $return_type = "row") {
        ...
        $client_po_info = $this->Clients_po_list_model->get_one($data->task_id);
        $client_info = $this->Clients_model->get_one($client_po_info->vendor_id);
...
$delete = js_anchor("<i class='fa fa-times pull-right p3'></i>", array('title' => lang('delete_income'), "class" => "delete-checklist-item", "data-fade-out-on-success" => "#checklist-item-row-$data->id", "data-action-url" => get_uri("clients_po_list/delete_checklist_item/$data->id"), "data-action" => "delete-confirmation"));
    ...
    $task_id = $this->Clients_po_payments_list_model->get_one($id)->task_id;
...
$save_id = $this->Clients_po_payments_list_model->save($data, $id);
    ...
    function save_checklist_items_sort() {
        ...
        if ($sort_values) {
            ...
            $this->Clients_po_payments_list_model->save($data, $id);
            }
        }
    }
    ...
    function delete_checklist_item($id) {
        ...
        if ($this->Clients_po_payments_list_model->delete($id)) {
    ...
    function get_vendors_invoice_paid_suggestion() {
        $item = $this->Clients_po_list_model->get_vendors_invoice_paid_amount_suggestion($this->input->post("item_name"));
        if ($item) {
            echo json_encode(array("success" => true, "item_info" => $item));
        } else {
            echo json_encode(array("success" => false));
...
$loan_voucher_no=str_ireplace("]",")",$vv);
    ...
    $loan_voucher_no="('empty')";
        ...
        $itemss = $this->Clients_po_list_model->get_purchase_orderid($this->input->post("vendor_member"),$loan_voucher_no);
         $suggestions = array();
