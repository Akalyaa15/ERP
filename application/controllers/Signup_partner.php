...
class Signup_partner extends CI_Controller {
    ...
    function partner() {
        ...
        $view_data["type"] = "client";
        $view_data["signup_type_partner"] = "new_partner";
        $view_data["signup_message_partner"] = lang("create_an_account_as_a_new_partner");
        $this->load->view("signup/partner_index", $view_data);
    }
...
$user_data["job_title"] = "";
            ...
            //create a client
             $client_id = $this->Partners_model->save($client_data);
...
foreach ($query1->result() as $rows)
 ...
 $DB4->where('id', $c);
 $DB4->update('clients', array('partner_id' => $b));

 $DB5 = $this->load->database('default', TRUE);

 $DB5->where('id', $b);
 $DB5->update('partners', array('client_id' => $c));
 ...
 $DB8 = $this->load->database('default', TRUE);
 $DB8->select ("id");
 $DB8->from('partners');
 $DB8->order_by('id','desc');
 $DB8->limit(1);
...
$DB7->where('id', $d);
 $DB7->update('users', array('partner_id' => $e));
