...
class Rm_members extends MY_Controller {
...
private function can_view_team_members_social_links() {
    ...
    public function index() {
       ...
       if ($this->login_user->is_admin == "1")
        {
            //$this->template->rander("rm_members/index", $view_data);
            $this->template->rander_scroll("rm_members/index", $view_data);
        }
    ...
    function modal_form() {
        ...
        $this->load->view('rm_members/modal_form', $view_data);
    }

    /* save new member */

    function add_rm_member() {
        $this->access_only_admin();
    ...
    function invitation_modal() {
        $this->access_only_admin();
        $this->load->view('rm_members/invitation_modal');
    }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $row_data = array(
            $user_avatar,
            get_rm_member_profile_link($data->id, $full_name),
            $data->job_title,
    ...
    function view($id = 0, $tab = "") {
        if ($id * 1) {
            ...
            $options = array("id" => $id, "user_type" => "resource");
            ...
            if ($user_info) {
                ...
                $this->template->rander("rm_members/view", $view_data);
            } else {
                show_404();
            }
        } else {
            ...
            $view_data['team_members'] = $this->Users_model->get_details(array("user_type" => "resource", "status" => "active"))->result();
            $this->template->rander("rm_members/profile_card", $view_data);
        }
    }
    ...
    function job_info($user_id) {
        ...
        $this->load->view("rm_members/job_info", $view_data);
    }
    ...
    function general_info($user_id) {
        ...
        $this->load->view("rm_members/general_info", $view_data);
    }
    ...
    function save_my_preferences() {
        //setting preferences
        $settings = array("notification_sound_volume","hidden_topbar_menus");
    ...
    function projects_info($user_id) {
        if ($user_id) {
            ...
            $this->load->view("rm_members/projects_info", $view_data);
        }
    }
    ...
    function attendance_info($user_id) {
        if ($user_id) {
            $view_data['user_id'] = $user_id;
            $this->load->view("rm_members/attendance_info", $view_data);
        }
    }

    //show weekly attendance list of a team member
    function weekly_attendance() {
        $this->load->view("rm_members/weekly_attendance");
    }

    //show weekly attendance list of a team member
    function custom_range_attendance() {
        $this->load->view("rm_members/custom_range_attendance");
    }

    //show attendance summary of a team member
    function attendance_summary($user_id) {
        $view_data["user_id"] = $user_id;
        $this->load->view("rm_members/attendance_summary", $view_data);
    }
    ...
    function leave_info($applicant_id) {
        if ($applicant_id) {
            ...
            $this->load->view("rm_members/leave_info", $view_data);
        }
    }

    //show yearly leave list of a team member
    function yearly_leaves() {
        $this->load->view("rm_members/yearly_leaves");
    }

    //show yearly leave list of a team member
    function expense_info($user_id) {
        $view_data["user_id"] = $user_id;
        $this->load->view("rm_members/expenses", $view_data);
    }
    ...
    function files($user_id) {
        ...
        $view_data['user_id'] = $user_id;
        $this->load->view("rm_members/files/index", $view_data);
    }
    ...
    function file_modal_form() {
        ...
        $view_data['user_id'] = $user_id;
        $this->load->view('rm_members/files/modal_form', $view_data);
    }
    ...
    private function _make_file_row($data) {
        ...
        $uploaded_by = get_rm_member_profile_link($data->uploaded_by, $uploaded_by);
        ...
        $options = anchor(get_uri("rm_members/download_file/" . $data->id), "<i class='fa fa fa-cloud-download'></i>", array("title" => lang("download")));

        $options .= js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_file'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("rm_members/delete_file"), "data-action" => "delete-confirmation"));
    ...
    function view_file($file_id = 0) {
        ...
        if ($file_info) {
            ...
            $this->load->view("rm_members/files/view", $view_data);
        } else {
            show_404();
        }
    }
