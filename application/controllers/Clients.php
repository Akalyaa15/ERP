...
class Clients extends MY_Controller {
    ...
    function index() {
        ...
        $this->template->rander_scroll("clients/index", $view_data);
    }
...
private function _get_groups_dropdown_select2_data($show_header = false) {
 ...
 if ($name>0) {
    ...
    function save() {
        ...
        // Ensure enable_client_logo has a value
        $enable_client_logo = $this->input->post('enable_client_logo') ? $this->input->post('enable_client_logo') : 0;
         $data = array(
            ...
            "enable_client_logo" => $enable_client_logo,
        ...
        function is_duplicate_company_name($company_name, $id = 0) {
            ...
            $sql = "SELECT id 
                    ...
                    AND deleted = 0 
                    AND id != ?";
            $result = $this->db->query($sql, [$company_name, $id]);
    ...
    function bank_info_tab($client_id = 0) {
        if ($client_id) {
            ...
            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $this->load->view('clients/bank_info/bank_info', $view_data);
        }
    }
...
function clients_excel_form() {

        $this->load->view('clients/clients_excel_form');
    }
...
$list_datas = $this->Clients_model->get_import_detailss($options)->row();
...
//clients po list
function clients_po_list($client_id) {
        $this->access_only_allowed_members();

        if ($client_id) {
            $view_data["client_info"] = $this->Clients_model->get_one($client_id);
            $view_data['client_id'] = $client_id;
            $this->load->view("clients/client_po_list", $view_data);
        }
    }


 //clients wo list
function clients_wo_list($client_id) {
        $this->access_only_allowed_members();

        if ($client_id) {
            $view_data["client_info"] = $this->Clients_model->get_one($client_id);
            $view_data['client_id'] = $client_id;
            $this->load->view("clients/client_wo_list", $view_data);
        }
    }
