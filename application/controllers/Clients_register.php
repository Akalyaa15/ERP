...
class Clients_register extends MY_Controller {
 ...
 $this->load->library('excel');
    ...
    function index() {
        ...
        //$this->template->rander("clients_register/index", $view_data);
        $this->template->rander_scroll("clients_register/index", $view_data);
    }
    ...
    function modal_form() {
       ...
       // $view_data['state_dropdown'] = $this->_get_state_dropdown_select2_data();
        ...
        $this->load->view('clients_register/modal_form', $view_data);
    }
...
private function _get_groups_dropdown_select2_data($show_header = false) {
 ...
 $DB2->from('clients');
 $DB2->where('id',$client_id);
 $DB2->where('partner_id >',0);
 $DB2->where('deleted',0);
  $query3=$DB2->get();
 ...
 if ($name>0) {
    ...
    $client_groups = $this->Client_groups_model->get_all_where(array("deleted" => 0, "status" => "active"))->result();
        $groups_dropdown = array();
}
    ...
    function save() {
        ...
        $data = array(
            ...
            "enable_client_logo" =>$this->input->post('enable_client_logo'),
            "state_mandatory"=>$this->input->post('state_mandatory'),
        );

        $client_info_logo = $this->Clients_model->get_one($client_id);
        ...
        if ($client_logo && !$client_logo_file) {
            ...
            $new_files =delete_file_from_directory(get_general_file_path("client", $client_id) . $client_logo_file);
            $data["client_logo"] = $value;
        }
       ...
       /* if (get_setting("disallow_duplicate_client_company_name") == "1" && $this->Clients_model->is_duplicate_company_name($data["company_name"], $client_id)) {*/
        if ($this->Clients_model->is_duplicate_company_name($data["company_name"], $client_id)) {
 ...
 $DB4 ->where('client_id',$client_id);
 $DB4->update('partners', array("company_name" => $company_name,
            "address" => $this->input->post('address'),
            "city" => $this->input->post('city'),
            "state" => $this->input->post('state'),
            "zip" => $this->input->post('zip'),
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $row_data = array($data->id,
            anchor(get_uri("clients_register/view/" . $data->id), $data->company_name),
        ...
        $row_data[] = modal_anchor(get_uri("clients_register/modal_form"), "<i class='fa fa-pencil'></i>", array("class" => "edit", "title" => lang('edit_client'), "data-post-id" => $data->id))
                . js_anchor("<i class='fa fa-times fa-fw'></i>", array('title' => lang('delete_client'), "class" => "delete", "data-id" => $data->id, "data-action-url" => get_uri("clients_register/delete"), "data-action" => "delete-confirmation"));
    ...
    function view($client_id = 0, $tab = "") {
        ...
        if ($client_id) {
            ...
            if ($client_info) {
                ...
                $this->template->rander("clients_register/view", $view_data);
            } else {
                show_404();
            }
        } else {
    ...
    function contact_profile($contact_id = 0, $tab = "") {
        ...
        if ($view_data['user_info']->user_type === "client") {
            ...
            $this->template->rander("clients_register/contacts/view", $view_data);
        } else {
            show_404();
        }
    }
    ...
    function my_preferences() {
        ...
        $this->load->view("clients_register/contacts/my_preferences", $view_data);
    }
    ...
    function contacts($client_id) {
        ...
        if ($client_id) {
            ...
            $this->load->view("clients_register/contacts/index", $view_data);
        }
    }
    ...
    function add_new_contact_modal_form() {
        ...
        $this->load->view('clients_register/contacts/modal_form', $view_data);
    }
    ...
    function contact_general_info_tab($contact_id = 0) {
        if ($contact_id) {
            ...
            $this->load->view('clients_register/contacts/contact_general_info_tab', $view_data);
        }
    }
    ...
    function company_info_tab($client_id = 0) {
        ...
        $view_data['state_dropdown'] = $state_categories_suggestion;
            ...
            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $this->load->view('clients_register/contacts/company_info_tab', $view_data);
        }
    }

    function bank_info_tab($client_id = 0) {
        if ($client_id) {
            //$this->access_only_allowed_members_or_client_contact($client_id);
            ...
            $view_data['label_column'] = "col-md-2";
            $view_data['field_column'] = "col-md-10";
            $this->load->view('clients_register/bank_info/bank_info', $view_data);
        }
    }

    function save_bank_info($client_id) {
     ...
     //$this->access_only_allowed_members_or_client_contact($client_id);
       ...
       // ));
        ...
        $user_info_updated = $this->Clients_model->save($user_data, $client_id);
    ...
    function invitation_modal() {
        ...
        $view_data["client_info"] = $this->Clients_model->get_one($client_id);
        $this->load->view('clients_register/contacts/invitation_modal', $view_data);
    }
    ...
    function users() {
        if ($this->login_user->user_type === "client") {
            ...
            $this->template->rander("clients_register/contacts/users", $view_data);
        }
    }
