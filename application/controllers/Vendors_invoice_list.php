...
class Vendors_invoice_list extends MY_Controller {
...
//$this->access_only_allowed_members();
 ...
 /*   //get team members dropdown
    ...
    function save() {
        ...
        if (!$id) {
            // Check if the vendor invoice number already exists
        ...
        if ($id) {
            // Check if the vendor invoice number already exists for an existing record
            $data["invoice_no"] = $this->input->post('invoice_no');
        ...
        if ($save_id) {
            // If new record, process purchase order payments
            if (!$id && $purchase_order_id) {
                $purchase_options = array("purchase_order_id" => $purchase_order_id);
                $purchase_payment_list = $this->Purchase_order_payments_model->get_details($purchase_options)->result();
    
                if ($purchase_payment_list) {
                    foreach ($purchase_payment_list as $purchase_payment_list_data) {
                        $payment_data = array(
                            "task_id" => $save_id,
 ...
 /* checklist */
    ...
    exit();
        ...
        if ($save_id) {
            ...
            // purchase orders payment list save the vendors invoice payment list
       
          
        $purchase_payment_list = $this->Vendors_invoice_payments_list_model->get_one($save_id); 
        ...
        $vendor_list_data = $this->Vendors_invoice_list_model->get_one($purchase_payment_list->task_id);
             
             $payment_data = array(
            "purchase_order_id" => $vendor_list_data->purchase_order_id,
            "payment_date" =>  $purchase_payment_list->payment_date,
            "payment_method_id" =>$purchase_payment_list->payment_method_id,
            "note" => $purchase_payment_list->description,
            "amount" => $purchase_payment_list->title,
            "reference_number" => $purchase_payment_list->reference_number,
            "created_at" => get_current_utc_time(),
            "created_by" => $this->login_user->id,
            "files"=> $purchase_payment_list->files,
        

        );
              $vendor_invoice_payment_save_id = $this->Purchase_order_payments_model->save($payment_data);
             $po_info = $this->Purchase_order_payments_model->get_one($vendor_invoice_payment_save_id);
              if($po_info){
            
                $po_payment_data = array(
            
            "po_payment_id"=>$po_info->id,
            "purchase_order_id"=>$po_info->purchase_order_id,
            );
                $vi_payment_save_id = $this->Vendors_invoice_payments_list_model->save($po_payment_data, $save_id);
        ...
        // end purchase orders payment list save the vendors invoice payment list
    ...
    function delete_checklist_item($id) {
      ...
      /*  if ($id) {
        ...
        if ($this->Vendors_invoice_payments_list_model->delete($id)) {
            //  delete  the vendor invoice payments list 
            
               $purchase_payment_list_data = $this->Vendors_invoice_payments_list_model->get_one($id);    
               
               $DB4 = $this->load->database('default', TRUE);

               $DB4 ->where('id',$purchase_payment_list_data->po_payment_id);
               $DB4->update('purchase_order_payments', array("deleted"=> 1));
