    ...
    <div class="p10 clearfix">
        <div class="media m0 bg-white">
            ...
            <div class="media-body w100p pt5">
                ...
                <p>
                    <?php
                    // Check if the points property exists, otherwise use a default value
                    $points = isset($model_info->points) ? $model_info->points : 0;  // default to 0 if not set
                    ?>
                    <span class='label label-light mr5' title='Point'><?php echo $points; ?></span>


                    <?php
                    $labels = "Your Label Text"; // Set a default value for $labels
                    $status_key_name = isset($model_info->status_key_name) ? $model_info->status_key_name : '';
                    $status_title = isset($model_info->status_title) ? $model_info->status_title : '';
                    $status_color = isset($model_info->status_color) ? $model_info->status_color : '#000'; // default to black if not set

                    echo $labels . " " . "<span class='label' style='background:$status_color; '>" . ($status_key_name ? lang($status_key_name) : $status_title) . "</span>";
                    ?>
    ...
    <div id="expense-dropzone" class="post-dropzone">
        <div class="col-md-12 mb15 b-t">
            ...
            <div class="form-group">
                ...
                <div class="col-md-9">
                    <?php
                    $payment_method_id = isset($model_info->payment_method_id) ? $model_info->payment_method_id : '';

                    echo form_dropdown(
                        "payment_method_id",
                        $payment_methods_dropdown,
                        array($payment_method_id),
                        "class='select2 validate-hidden' id='payment_method_id' data-rule-required='true' data-msg-required='" . lang('field_required') . "'"
                    );
                    ?>
        ...
        $("#loan-payment-table").appTable({
            ...
            columns: [
                ...
                },
                {
                    ...
                    "class": "text-center w25p"
                },
                {
                    title: '<?php echo lang("reference_number") ?>',
                    "class": "text-center w25p"
                },
                {
