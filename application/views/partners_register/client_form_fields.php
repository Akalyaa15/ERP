...
$country_dummy_name = isset($country_id_name->countryName) ? $country_id_name->countryName : '';

// Check if model_info has the properties before using them
$model_info_id = isset($model_info->id) ? $model_info->id : '';
$model_info_partner_id = isset($model_info->partner_id) ? $model_info->partner_id : '';
$model_info_company_name = isset($model_info->company_name) ? $model_info->company_name : '';
?>

<input type="hidden" name="id" value="<?php echo htmlspecialchars($model_info_id, ENT_QUOTES, 'UTF-8'); ?>" />
<input type="hidden" id="country_code_id" value="" />
<input type="hidden" name="partner_id" value="<?php echo htmlspecialchars($model_info_partner_id, ENT_QUOTES, 'UTF-8'); ?>" />
<input type="hidden" name="view" value="<?php echo isset($view) ? htmlspecialchars($view, ENT_QUOTES, 'UTF-8') : ""; ?>" />

<div class="form-group">
    <label for="company_name" class="<?php echo $label_column; ?>"><?php echo lang('partner_name'); ?></label>
    <div class="<?php echo $field_column; ?>">
        <?php
        echo form_input(array(
            "id" => "company_name",
            "name" => "company_name",
            "value" => htmlspecialchars($model_info_company_name, ENT_QUOTES, 'UTF-8'),
            "class" => "form-control",
            "placeholder" => lang('partner_name'),
            "autofocus" => true,
...
<?php if ($this->login_user->user_type === "staff") { ?>
    <div class="form-group">
        <label for="groups" class="<?php echo $label_column; ?>"><?php echo lang('partner_groups'); ?></label>
        <div class="<?php echo $field_column; ?>">
            <?php
            echo form_input(array(
                "id" => "group_ids",
                ...
                "class" => "form-control",
                "placeholder" => lang('partner_groups')
            ));
            ?>
        </div>
    </div>
...
/* $("#gst_number").on('keyup',function () {
    $("#gstin_number_first_two_digits").val("").attr('readonly', false)
          ...
          $("#gstin_number_first_two_digits").select2({
            showSearchBox: true,
            ajax: {
                url: "<?php /* echo get_uri("clients/get_gst_state_suggestion"); */ ?>",
                dataType: 'json',
