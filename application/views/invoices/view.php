...
?>
<?php $options = array("invoice_id" => $invoice_info->id);
            $invoice_items = $this->Invoice_items_model->get_details($options)->row();
           ?>
<div id="page-content" class="clearfix">
    <div style="max-width: 1000px; margin: auto;">
        <div class="page-title clearfix mt15">
            <h1><?php echo /*get_invoice_id($invoice_info->id)*/ $invoice_info->invoice_no ?$invoice_info->invoice_no:get_invoice_id($invoice_info->id); ?>
                <?php
...
{ ?>
                        <?php echo anchor(get_uri("invoices/download_pdf/" . $invoice_info->id), "<i class='fa fa-download'></i> " . lang('download_pdf'), array("title" => lang('download_pdf'))); ?> <?php } else { ?>
                        <?php echo anchor(get_uri("invoices/download_invoice_without_gst_pdf/" . $invoice_info->id), "<i class='fa fa-download'></i> " . lang('download_pdf'), array("title" => lang('download_pdf'))); ?>
                        <?php } ?>
                   ...
                   <li role="presentation">
                        ...
                        <li role="presentation">
<?php if ($hsn_size>0||$invoice_total_summary->freight_tax_amount||$invoice_total_summary->installation_tax)
{ ?>
                        <?php echo anchor(get_uri("invoices/download_print_pdf/" . $invoice_info->id), "<i class='fa fa-print'></i> " . lang('print_invoice_to_client'), array("title" => lang('download_print_pdf'))); ?> <?php } else { ?><?php echo anchor(get_uri("invoices/download_print_invoice_without_gst_pdf/" . $invoice_info->id), "<i class='fa fa-print'></i> " . lang('print_invoice_to_client'), array("title" => lang('print_invoice_to_client'))); ?><?php } ?> </li>
...
<?php if ($list_data) { ?>
                             <li role="presentation"><?php echo anchor(get_uri("delivery/view/" . $list_data->id), "<i class='fa fa-eye'></i> " . lang('view_dc'), array("data-reload-on-success" => "1")); ?> </li> <?php }else if($invoice_items){ ?>
                             <li role="presentation"><?php echo anchor(get_uri("delivery/create_dc_from_invoice/" . $invoice_info->id), "<i class='fa fa-refresh'></i> " . lang('create_dc'), array("data-reload-on-success" => "1")); ?> </li>
                        <?php } ?>  <?php } ?>

                        <?php if ($invoice_item_list_data && $invoice_status == "draft") { ?>
        ...
        $("#invoice-item-table").appTable({
            ...
            columns: [
               ...
               {title: '<?php echo lang("discount_percent") ?>', "class": "text-center w10p", "bSortable": false},
                {title: '<?php echo lang("discount_amount") ?>', "class": "text-right w10p", "bSortable": false},
