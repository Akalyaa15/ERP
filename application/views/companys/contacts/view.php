...
<div id="page-content" class="clearfix">
    <div class="row bg-dark-success p20">
        ...
        <div class="col-md-6">
            <p> 
                <?php
                // Check if $company_info is set and not null before using it
                if (isset($company_info) && $company_info !== null) {
                    ...
                    echo "<p class='text-danger'>" . lang("company_info_not_found") . "</p>";
                }
                ?>
            </p>
            <?php if (isset($company_info) && $company_info !== null && $company_info->address) { ?>
                <p><?php echo nl2br($company_info->address); ?>
                    ...
                    <?php if ($company_info->state) { ?>
                        <br />
                        <?php
                        if (!is_numeric($company_info->state)) {
                            $company_info->state = 0;
                        }
                        $options = array("id" => $company_info->state);
                        $state_id_name = $this->States_model->get_details($options)->row();
                        $state_dummy_name = isset($state_id_name->title) ? $state_id_name->title : "";
                    ...
                    <?php if ($company_info->country) { ?>
                        <br />
                        <?php
                        if (!is_numeric($company_info->country)) {
                            $company_info->country = 0;
                        }
                        $options = array("numberCode" => $company_info->country);
