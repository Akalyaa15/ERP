...
$freight_total=$querys_same_taxvalues->row();
 ...
 foreach ($querys->result() as $rowss) { 
    echo to_currency($installation_total_row->installation_total+$rowss->total+
      $freight_total->amount, $estimate_total_summary->currency_symbol); } ?>
        </td>
...
if($hsn_sizes>0 || $estimate_total_summary->freight_tax_amount||
...
<?php 
$taxamt = 0;
if (isset($estimate_total_summary->estimate_tax_subtotal)) {
    $taxamt += $estimate_total_summary->estimate_tax_subtotal;
}
if (isset($estimate_total_summary->installation_tax)) {
    $taxamt += $estimate_total_summary->installation_tax;
}
if (isset($estimate_total_summary->freight_tax3)) {
    $taxamt += $estimate_total_summary->freight_tax3;
}
if ($taxamt) { ?><table style="width: 99%; color: #444;line-height: 15px">
