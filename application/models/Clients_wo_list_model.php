...
class Clients_wo_list_model extends Crud_model {

    private $table = null;

    function __construct() {
        $this->table = 'clients_wo_list';
        parent::__construct($this->table);
    }

    function get_details($options = array()) {
        $vendors_invoice_list_table = $this->db->dbprefix('clients_wo_list');
        $vendors_table = $this->db->dbprefix('clients');
        $task_status_table = $this->db->dbprefix('vendors_invoice_status');
         $vendors_invoice_payment_list_table = $this->db->dbprefix("clients_wo_payments_list");
...
$status = get_array_value($options, "status");
    ...
    function get_vendors_invoice_paid_amount_suggestion(
        $item_name = "") {
        $hsn_sac_code_table = $this->db->dbprefix('clients_wo_payments_list');
