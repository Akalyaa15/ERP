...
class Groups_comments_model extends Crud_model {
    ...
    function get_details($options = array()) {
        $project_comments_table = $this->db->dbprefix('group_comments');
        ...
        $sql = "SELECT $project_comments_table.*, $project_comments_table.id AS parent_commment_id, CONCAT($users_table.first_name, ' ',$users_table.last_name) AS created_by_user, $users_table.image as created_by_avatar, $users_table.user_type,
            (SELECT COUNT($project_comments_table.id) as total_replies FROM $project_comments_table WHERE $project_comments_table.deleted=0 AND $project_comments_table.comment_id=parent_commment_id) AS total_replies
    ...
    function save_comment($data) {
        ...
        if ($comment_id) {
            ...
            $group_members = $this->Groups_model->get_one($comment_info->project_id);
        ...
        $array_string_group_members =  implode(",",$convert_array);
            ...
            $data["group_members"] = $array_string_group_members;
            $type = "";
            $type_id = "";
    ...
    function count_group_unread_message($user_id = 0,$group_id = 0) {
        $messages_table = $this->db->dbprefix('group_comments');

        $sql = "SELECT COUNT($messages_table.id) as total
        FROM $messages_table
        WHERE $messages_table.deleted=0  AND FIND_IN_SET($user_id, $messages_table.group_members) != 0 AND FIND_IN_SET($user_id, $messages_table.read_members) = 0 AND $messages_table.project_id =$group_id  ";
        return $this->db->query($sql)->row()->total;
    }

    function set_group_message_status_as_read($project_id, $user_id = 0) {
        $messages_table = $this->db->dbprefix('group_comments');
        //$sql = "UPDATE $messages_table SET read_members=$user_id WHERE FIND_IN_SET($user_id, $messages_table.group_members) AND ($messages_table.project_id=$project_id)";
        $sql = "UPDATE $messages_table SET $messages_table.read_members = CONCAT($messages_table.read_members,',',$user_id)
        WHERE $messages_table.project_id=$project_id AND FIND_IN_SET($user_id, $messages_table.read_members) = 0 AND FIND_IN_SET($user_id, $messages_table.group_members)";
        return $this->db->query($sql);
    }
