    ...
    function create_notification($event, $user_id, $options = array()) {
      ...
      $purchase_order_id = get_array_value($options, "purchase_order_id");
       ...
       $group_comment_id = get_array_value($_POST, "group_comment_id");
        ...
        if (in_array("voucher_application_submitted", $notify_to_terms) && $voucher_id) {
            $where .= " OR ($users_table.id=(SELECT $voucher_table.line_manager FROM $voucher_table WHERE $voucher_table.id=$voucher_id)) ";
        }  
        if (in_array("line_manager", $notify_to_terms) && $voucher_id) {
$tab=$this->Voucher_model->get_one($voucher_id);
            if($tab->line_manager=="admin"){
            $where .= " OR FIND_IN_SET($users_table.id, (SELECT GROUP_CONCAT($users_table.id) AS contact_userss FROM $users_table WHERE $users_table.is_admin='1')) ";
            }else{
            $where .= " OR ($users_table.id=(SELECT $voucher_table.line_manager FROM $voucher_table WHERE $voucher_table.id=$voucher_id)) ";
            }        } 
     if (in_array("voucher_creater", $notify_to_terms) && $voucher_id) {
            $where .= " OR ($users_table.id=(SELECT $voucher_table.created_user_id FROM $voucher_table WHERE $voucher_table.id=$voucher_id)) ";
        }         
        if (in_array("voucher_application_paid", $notify_to_terms) && $voucher_id) {
            $where .= " OR ($users_table.id=(SELECT $voucher_table.created_user_id FROM $voucher_table WHERE $voucher_table.id=$voucher_id)) ";
        }          if (in_array("voucher_application_rejected_by_manager", $notify_to_terms) && $voucher_id) {
            $where .= " OR ($users_table.id=(SELECT $voucher_table.created_user_id FROM $voucher_table WHERE $voucher_table.id=$voucher_id)) ";
        } 
                if (in_array("voucher_application_rejected_by_accounts", $notify_to_terms) && $voucher_id) {
            $where .= " OR ($users_table.id=(SELECT $voucher_table.created_user_id FROM $voucher_table WHERE $voucher_table.id=$voucher_id)) ";
        }  
                if (in_array("voucher_application_approved_by_accounts", $notify_to_terms) && $voucher_id) {
            $where .= " OR ($users_table.id=(SELECT $voucher_table.created_user_id FROM $voucher_table WHERE $voucher_table.id=$voucher_id)) ";
        }       
                if (in_array("voucher_application_approved_by_manager", $notify_to_terms) && $voucher_id) {
            $where .= " OR ($users_table.id=(SELECT $voucher_table.created_user_id FROM $voucher_table WHERE $voucher_table.id=$voucher_id)) ";
        }
        if (in_array("account_department", $notify_to_terms) && $voucher_id) {
            $where .= " OR FIND_IN_SET($users_table.id, (SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.department='09')) ";
        ...
        if (in_array("hr_department", $notify_to_terms) && $leave_id) {
            $where .= " OR FIND_IN_SET($users_table.id, (SELECT GROUP_CONCAT($users_table.id) AS contact_users FROM $users_table WHERE $users_table.department='07')) ";
        }

          if (in_array("admin", $notify_to_terms) && $leave_id) {
            $where .= " OR FIND_IN_SET($users_table.id, (SELECT GROUP_CONCAT($users_table.id) AS contact_userss FROM $users_table WHERE $users_table.is_admin='1')) ";
        }
        ...
        //find leave_manager
        if (in_array("leave_manager", $notify_to_terms) && $leave_id) {
 $tab=$this->Leave_applications_model->get_one($leave_id);
            if($tab->line_manager=="admin"){
            $where .= " OR FIND_IN_SET($users_table.id, (SELECT GROUP_CONCAT($users_table.id) AS contact_userss FROM $users_table WHERE $users_table.is_admin='1')) ";
            }else{
            $where .= " OR ($users_table.id=(SELECT $leave_applications_table.line_manager FROM $leave_applications_table WHERE $leave_applications_table.id=$leave_id)) ";
            }        }
        //find leave_manager
        if (in_array("leave_alternate", $notify_to_terms) && $leave_id) {
            $where .= " OR ($users_table.id=(SELECT $leave_applications_table.alternate_id FROM $leave_applications_table WHERE $leave_applications_table.id=$leave_id)) ";
        }  
     ...
     /*   if (in_array("recipient", $notify_to_terms) && $announcement_id) {
        ...
        //find group members for group message
        if (in_array("group_members", $notify_to_terms) && $group_id) {
            $where .= " OR FIND_IN_SET($users_table.id, (SELECT GROUP_CONCAT($groups_table.members) AS grou_users FROM $groups_table WHERE $groups_table.deleted=0 AND $groups_table.id=$group_id)) ";
        }

         //find group comment creator, comment id is not = id. It should be the the original comment_id
        if (in_array("group_comment_creator", $notify_to_terms) && $group_comment_id) {
            $where .= " OR ($users_table.id=(SELECT $group_comments_table.created_by FROM $group_comments_table WHERE $group_comments_table.id=$group_comment_id)) ";
        }
        ...
        $data = array(
            ...
            "group_comment_id" => $group_comment_id? $group_comment_id:"",
        );
